...
function mapSummary(records) {
    ...
    records.forEach((record) => {
        ...
        if (sessionStatus === 'confirmed') summary.confirmed += 1;
        if (sessionStatus === 'pending') summary.pending += 1;
        if (sessionStatus === 'failed' || sessionStatus === 'conflict') summary.failed += 1;
        if (paymentStatus === 'paid' || paymentStatus === 'captured' || paymentStatus === 'confirmed') summary.paid += 1;
    });
...
export async function onRequestGet({ request, env }) {
    const githubToken = parseBearerToken(request);
    if (!githubToken) {
        return jsonResponse({ error: 'Missing Authorization bearer token.' }, 401);
    }

    const repoOwner = env.GITHUB_REPO_OWNER || 'Emmanueld14';
    const repoName = env.GITHUB_REPO_NAME || 'bloomly-app';
    const access = await verifyGitHubAccess(githubToken, repoOwner, repoName);
    if (!access.ok) {
        return jsonResponse({ error: access.error || 'Unauthorized.' }, access.status || 401);
    }
    ...
    const start = isValidDate(url.searchParams.get('start')) ? url.searchParams.get('start') : startDefault.toISOString().slice(0, 10);
    const end = isValidDate(url.searchParams.get('end')) ? url.searchParams.get('end') : now.toISOString().slice(0, 10);
    ...
    try {
        ...
        return jsonResponse({ error: error.message || 'Unable to load records.' }, 500);
    }
}
